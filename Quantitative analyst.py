...
class Config:
    ...
    # 新增圖表繪製開關 (None: 繪製所有圖表; []: 不繪製任何圖表; ["MSFT", "AAPL"]: 繪製指定股票圖表)
    TICKERS_TO_PLOT = [] # 預設為 None，表示繪製所有圖表

# ==========================================
# 3. 核心數據引擎 - 加入 MACD 交叉訊號
# ==========================================
...
def plot_dashboard(df, ticker, should_plot=True):
    ...
    print("\n" + "="*70)
    print(f"📊 {ticker} 量化戰情室報告 ({last_idx.date()})")
    print("="*70)
    print(f"1. 市場微結構 (HMM):  {hmm_labels[current_state]}")
    print(f"2. 目前股價:          ${last_row['Close']:.2f}")
    print(f"3. 動能指標 (MA10):   ${last_row['MA_10']:.2f} (Gap: {dist_to_ma10:+.2f}%)")
    print(f"4. 機構成本 (MA50):   ${last_row['MA_50']:.2f} (Gap: {dist_to_ma50:+.2f}%)")
    print(f"5. RSI (14):          {last_row['RSI']:.1f}")
    print(f"6. MACD 狀態:         MACD {last_row['MACD']:+.3f} | Signal {last_row['MACD_Signal']:+.3f} | Hist {last_row['MACD_Hist']:+.3f}")
    ...
    if should_plot:
        ...
        ax1.set_title(f"{ticker} 多因子策略儀表板 (HMM + MA + RSI + MACD 優化版)", fontsize=14)
...
# ==========================================
# 6. 歷史數據與排程管理 (New Features)
# ==========================================
def save_daily_snapshot(df):
    """
    保存每日分析快照到 CSV
    1. 增加 Date 欄位 (YYYY-MM-DD)
    2. 追加到 history_data.csv
    3. 避免重複寫入同一天的數據
    """
    ...
    if os.path.exists(file_path):
        try:
            ...
            if not existing_df[existing_df['Date'] == today_str].empty:
                print(f"⚠️ {today_str} 的數據已存在，將覆蓋今日數據...")
                # 剔除今日舊數據，保留其他日期的
    ...
    print(f"✅ 已保存 {today_str} 的分析結果到 {file_path}")
...
def should_run_now():
    ...
    檢查是否應該執行腳本：
    1. 今天是否為 NYSE 交易日
    2. 現在時間是否超過收盤時間 (16:15 ET)
    """
    # 設定時區
    eastern = pytz.timezone('US/Eastern')
    now_et = datetime.now(eastern)
    
    print(f"🕒 當前美東時間: {now_et.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if now_et.time() < target_time.time():
        print(f"⏳ 尚未收盤 (目標時間 {target_time.time()})，稍後再試。")
        return False
...
if __name__ == "__main__":
    if should_run_now():
        all_summaries = []
        print(f"\n🚀 開始批量分析 {len(Config.TICKERS)} 個標的：{Config.TICKERS}\n")
        ...
        for ticker in Config.TICKERS:
            ...
            if data is not None and len(data) > 200:
                ...
                # 注意：如果是在自動化環境跑，可以把 plot_dashboard 的 should_plot 強制設為 False 以節省資源
                # summary = plot_dashboard(data_analyzed, ticker, should_plot=should_plot_for_ticker)
                
                # 為了避免 GitHub Action 報錯，這裡預設保留原有邏輯，但建議在 headless 環境設為 False
                summary = plot_dashboard(data_analyzed, ticker, should_plot=should_plot_for_ticker)
                
                all_summaries.append(summary)
            else:
                print(f"❌ {ticker} 數據不足，跳過分析。\n")
